...
const generateResponse = (input: string): string => {
  ...
  if (
    lowerInput.match(
      /(what is ridewise|about ridewise|what does ridewise do|tell me about|this website|this platform|this app)/
    )
  ) {
  ...
  if (
    lowerInput.match(
      /(daily prediction|daily forecast|predict daily|day prediction|full day|entire day)/
    )
  ) {
  ...
  if (
    lowerInput.match(
      /(hourly prediction|hourly forecast|hour prediction|per hour|by hour|peak hour)/
    )
  ) {
  ...
  if (
    lowerInput.match(
      /(weather|temperature|humidity|wind|rain|factors|affect|impact|influence)/
    )
  ) {
  ...
  if (
    lowerInput.match(
      /(comfort index|comfort level|feels like|apparent temperature)/
    )
  ) {
  ...
  // Accuracy
  if (
    lowerInput.match(/(accuracy|accurate|reliable|how good|precision|correct)/)
  ) {
  ...
  if (
    lowerInput.match(
      /(how to use|how do i|getting started|start|begin|tutorial|guide)/
    )
  ) {
  ...
  // Thanks / Appreciation
  if (
    lowerInput.match(/(thank|thanks|appreciate|helpful|great|awesome|good job)/)
  ) {
  ...
  if (
    lowerInput.match(
      /(machine learning|ml|algorithm|model|ai|artificial intelligence)/
    )
  ) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content:
        "Hi! 👋 I'm the RideWise Assistant. I can help you understand how our bike rental predictions work, explain weather factors, guide you through the platform, and answer any questions you have. You can type or use the 🎤 microphone button to speak! What would you like to know?",
      role: "assistant",
      timestamp: new Date(),
    },
  ]);
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 p-4 rounded-full gradient-primary text-primary-foreground shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-all duration-300 hover:scale-110 group ${
          isOpen ? "hidden" : "flex"
        }`}
      >
            ...
            >
              <Input
                ...
                placeholder={
                  isListening
                    ? "Listening..."
                    : "Ask about predictions, weather, features..."
                }
